...
void ConfigureEPWM(void)
{
    ...
    EPwm4Regs.CMPA.bit.CMPA = 300;     // Set compare A value to 2048 counts
//    EPwm4Regs.TBPRD = 2275;             // 30Hz
//    EPwm4Regs.TBPRD = 1143;             // 60Hz
    EPwm4Regs.TBPRD = 579;             // 60Hz
...
//    EPwm2Regs.CMPA.bit.CMPA = (0.1 + (0.8 * AdcaResultRegs.ADCRESULT0/4095))* 2500 ;
    ...
    if (val <= 360)
        {
        EPwm2Regs.CMPA.bit.CMPA = (sin( val*pi/180)+1)*EPwm2Regs.TBPRD/2;
        val++;
        }
        else
        {
